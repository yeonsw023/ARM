...
# 웹캠 스트리밍 (예: 카메라 인덱스가 1일 때)
cap = cv2.VideoCapture(1)  # 외장 웹캠을 위한 인덱스를 넣어줘

# 해상도 설정 (픽셀)
cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)  # 가로 해상도 (예: 1920px)
cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)  # 세로 해상도 (예: 1080px)
